...
function Upque() {
  ...
  const questSave = async (e) => {
    e.preventDefault();
    await axios.post("https://api.politekniklp3i-tasikmalaya.ac.id/kecerdasanquestions", {
      id_type: name,
      question: pertanyaan,
    })
      .then((response) => {
  ...
  const questDelete = async (id) => {
    ...
    if (konfirmasi) {
      await axios.delete(`https://api.politekniklp3i-tasikmalaya.ac.id/kecerdasanquestions/${id}`)
        .then((response) => {
          alert(response.data.message);
          getQuest();
          navigate('/');
  ...
  const questUpdate=async(e)=>{
    await axios.put(`https://api.politekniklp3i-tasikmalaya.ac.id/kecerdasanquestions/${id}`, {
      id_type: name,
      question: pertanyaan,
    })
      .then((response) => {
