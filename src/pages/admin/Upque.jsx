...
function Upque() {
  ...
  const questSave = async (e) => {
    e.preventDefault();
    await axios
      .post("http://localhost:8001/questions", {
        id_type: name,
        question: pertanyaan,
      })
      .then((response) => {
  ...
  const questDelete = async (id) => {
    ...
    if (konfirmasi) {
      await axios
        .delete(`http://localhost:8001/questions/${id}`)
        .then((response) => {
          alert(response.data.message);
          getQuest();
          navigate("/");
  ...
  const questUpdate = async (e) => {
    await axios
      .put(`http://localhost:8001/questions/${id}`, {
        id_type: name,
        question: pertanyaan,
      })
      .then((response) => {
  ...
  return (
    <div className="bg-sky-400 h-screen justify-center items-center p-4">
      <div className="bg-orange-50 p-4 rounded-xl">INPUT PERTANYAAN</div>
            ...
            <tbody>
              {questions.map((question, index) => (
                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                  ...
                  <td className="px-6 py-4">{question.id_type}</td>
                  <td className="px-6 py-4">{question.question}</td>
                  <td className="px-6 py-4">
                    <button
                      onClick={() => questDelete(question.id)}
                      className="m-4"
                    >
                      Hapus
                    </button>
                    <button
                      onClick={() => questUpdate(question.id, question.id_type)}
                      className="m-4"
                    >
                      Update Modal
                    </button>
